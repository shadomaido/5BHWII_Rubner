...
alle_Karten = [f"({suit}) {card}" for suit in suits for card in
               ["2", "3", "4", "5", "6", "7", "8", "9", "10", "B", "D", "K", "Ass"]]
...
def is_pair(ziehungen):
    return any(
        ziehungen[i] % 13 == ziehungen[j] % 13 for i in range(len(ziehungen)) for j in range(i + 1, len(ziehungen)))
...
def check_combinations(player, ziehungen, alle_Karten):
    ...
    formatted_card_names = [f"{colored_symbol(card.split()[1], card.split()[0])} {card.split()[0]}" for card in
                            card_names]
...
def poker_statistics():
    while True:
        ...
        for player in range(1, num_players + 1):
            ...
            result = check_combinations(player, gezogene_zahlen, alle_Karten)
            print(f"Person {player} hat: {result}")
...
def simulate_poker_game():
    ...
    percentages = {combo: (count / num_games) * 100 for combo, count in combinations_count.items()}
    return combinations_count, percentages



    return combinations_count

def calculate_percentage(combinations_count, num_games):
    percentages = {combo: (count / num_games) * 100 for combo, count in combinations_count.items()}
    return percentages
